  ...
  return (
    <aside className="sidebar">
      <button
        ...
        aria-label={`${polity.productName} home`}
      >
        <span className="brand-mark">{polity.shortName}</span>
        <span>
          <strong>CONNECT</strong>
          <small>{polity.jurisdictionName.toUpperCase()}</small>
        </span>
      </button>
      ...
      <div className="side-card">
        ...
        <p>
          Start with Problems & Ideas. Tell us what is happening in plain language. You do
          not need to know which City office, rule, or process applies.
        </p>
      </div>

      <div className="sidebar-footer">
        <button className="language-button">
          <CircleHelp size={16} /> English
        </button>
        <small>© 2026 {polity.productName}</small>
        <small>Built by OneTime Labs</small>
      </div>
    </aside>
  );
}
